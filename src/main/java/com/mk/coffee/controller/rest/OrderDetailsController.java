    ...
    public RestResult<OrderDetails> order(@RequestBody RequestCreateOrder createOrder) {
        ...
        if (createOrder.shoppingCartsItemIds == null) {
            if (EmptyUtils.isEmpty(createOrder.encryptCode) || EmptyUtils.isEmpty(createOrder.cardId)) {//没有微信卡券
                ...
                return RestResultGenerator.genSuccessResult(orderDetailsService.orderUseEncryptCode(createOrder.memberId,
                        createOrder.cardId, createOrder.encryptCode, createOrder.eNum));
            }
        } else {
            if (EmptyUtils.isEmpty(createOrder.encryptCode) || EmptyUtils.isEmpty(createOrder.cardId)) {
                return RestResultGenerator.genSuccessResult(orderDetailsService.order(createOrder.memberId,
                        createOrder.shoppingCartsItemIds, createOrder.eNum));
            } else {
                return RestResultGenerator.genSuccessResult(orderDetailsService.orderUseEncryptCode(createOrder.memberId,
                        createOrder.cardId, createOrder.encryptCode, createOrder.shoppingCartsItemIds, createOrder.eNum));
            }
        }
    }


    @PostMapping("/createTakeOutOrder")
    @ApiOperation(value = "生成外卖订单", notes = "根据成员id,shoppingCartsItemIds([1,2,3,4...id])," +
            "cardId，encryptCode（可选，使用微信卡券则必须同时传入） 微信卡券," +
            "coffeeMachineId（机器id,不是机器码），addressId（地址id）生成外卖订单")
    public RestResult<OrderDetails> createTakeOutOrder(@RequestBody RequestCreateTakeOutOrder takeOutOrder) {
        return RestResultGenerator.genSuccessResult(orderDetailsService.createTakeOutOrder(takeOutOrder));
    }
